...
if [ -z "$PI_RESET_PIN" ]; then
  ...
  if [ "$PI_RESET_PIN" == $defaultResetPin ]; then
    ...
    sed -i -e "s/SX1301_RESET_BCM_PIN=17/SX1301_RESET_BCM_PIN=${PI_RESET_PIN}/g" ./start.sh
  fi
fi
...
if [ -z "$GATEWAY_REGION" ]; then
  ...
  if [ "$GATEWAY_REGION" == $defaultRegion ]; then
    ...
    sed -i -e "s/router.eu.thethings/router.$region.thethings/g" ./local_conf.json
  fi
fi

# inject the values provided by the user into the template
#envsubst < local_conf.json > local_conf.json
